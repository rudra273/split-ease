...
export default function ExpenseDetailPage({ params }: { params: { id: string } }) {
  ...
  const addSplit = () => {
    ...
    setEditedExpense({
      ...editedExpense,
      splits: [...editedExpense.splits, { username: '', amount: 0 }]
    });
  };
        ...
        {isEditing ? (
                ...
                <div key={index} className="flex gap-4 mb-2">
                  ...
                  {editedExpense.split_type !== 'EQUAL' && (
                    <input
                      ...
                      required
                      min="0"
                      step={editedExpense.split_type === 'PERCENTAGE' ? '1' : '0.01'}
                      className="flex-1 border border-gray-300 rounded-md shadow-sm p-2"
                      value={editedExpense.split_type === 'PERCENTAGE' ? split.percentage : split.amount}
                      onChange={(e) => handleSplitChange(index, editedExpense.split_type === 'PERCENTAGE' ? 'percentage' : 'amount', e.target.value)}
                    />
                  )}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-700">${parseFloat(expense.amount).toFixed(2)}</p>
            </div>
              ...
              <div className="space-y-2">
                {expense.splits.map((split, index) => (
                  <div key={index} className="flex justify-between">
                    <span>Username: {split.username}</span>
                    <span>
                      {expense.split_type === 'PERCENTAGE'
                        ? `${parseFloat(split.percentage || '0').toFixed(2)}%`
